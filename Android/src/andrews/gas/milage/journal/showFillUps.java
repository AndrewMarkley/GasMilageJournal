...
public class showFillUps extends Activity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		ad.loadAd(adRequest);
		b4 = (ImageButton) findViewById(R.id.fill_up_button2);
		...
		for ( int ctr = 0; ctr < fillUps.size(); ctr++ ) {
			dates[ctr] = fillUps.get(ctr).getCarName() + "\t" + fillUps.get(ctr).getDate();
				...
				builder.setItems(options, new DialogInterface.OnClickListener()
				{
					...
					public void onClick(DialogInterface dialog, int p)
					{
						...
						if ( p == 0 ) {
							...
							for ( int ctr = 0; ctr < fillUps.size(); ctr++ )
								if ( fillUps.get(tempPos).getCarName().equals(fillUpsUnsorted.get(ctr).getCarName()) )
