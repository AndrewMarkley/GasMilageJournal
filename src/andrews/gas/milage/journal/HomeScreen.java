...
public class HomeScreen extends Activity
{
		...
		try {
			...
			for ( int ctr = 0; ctr < cars.size( ); ctr++ ) {
				...
				for ( int c = 0; c < temp.size( ); c++ )
					if ( temp.get( c ).getName( )
							.equals( cars.get( ctr ).getName( ) ) )
						maybe = false;
				if ( maybe )
		...
		save.setOnClickListener( new View.OnClickListener( )
		{
			...
			public void onClick(View v)
			{
				...
				try {
					...
					x = new FillUp( "" + car.getText( ), distance, gas,
							pricePG, cost, mpg, dateString.getText( )
									.toString( ).trim( ), comms );
	...
	private void updateDisplay()
	{
		if ( ! edit || counter > 0 )
			...
			dateString.setText( new StringBuilder( ).append( datee[0] )
					.append( "-" ).append( datee[1] ).append( "-" )
					.append( datee[2] ).append( " " ) );
		}
	}
