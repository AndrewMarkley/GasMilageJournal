...
public class HomeScreen extends Activity {
	...
	TextView milesPerGallon, gallonsPurchased, milesTraveled, pricePerGallon, totalCost, dateInput, dateString, comments = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
    	...
    	if(edit){
            ...
            temp = new FillUp(values[0], Double.parseDouble(values[1]), Double.parseDouble(values[2]), Double.parseDouble(values[3]), Double.parseDouble(values[4]), Double.parseDouble(values[5]), values[6], "");
    ...
    save.setOnClickListener(new View.OnClickListener() {
		...
		public void onClick(View v) {
        	...
        	try {
	        	...
	        	x = new FillUp(""+car.getText(), distance, gas, pricePG, cost, mpg, dateString.getText().toString().trim(), comms);
	...
	private void updateDisplay() {
	    if(!edit || counter>0)
			dateString.setText( new StringBuilder().append(mMonth + 1).append("-").append(mDay).append("-").append(mYear).append(" "));
	    else{
	    	String x = values[6];
	    	...
	    	dateString.setText( new StringBuilder().append(datee[0]).append("-").append(datee[1]).append("-").append(datee[2]).append(" "));
	    }
	}
