...
public class DatabaseHandler extends SQLiteOpenHelper
{
	...
	public void onCreate(SQLiteDatabase db)
	{
		String CREATE_FILL_UP_TABLE = "CREATE TABLE " + TABLE_FILL_UPS + "("
				+ KEY_CAR + " TEXT," + KEY_DISTANCE + " DOUBLE," + KEY_GAS
				+ " DOUBLE," + KEY_PRICE + " DOUBLE," + KEY_TOTAL_COST
				+ " DOUBLE," + KEY_MPG + " DOUBLE," + KEY_DATE + " TEXT,"
				+ KEY_COMMENTS + " TEXT" + ")";
	...
	FillUp getFillup(int id)
	{
		...
		Cursor cursor = db.query ( TABLE_FILL_UPS , new String[] { KEY_CAR,
				KEY_DISTANCE, KEY_GAS, KEY_PRICE, KEY_TOTAL_COST, KEY_MPG,
				KEY_DATE, KEY_COMMENTS } , KEY_CAR + "=?" ,
				new String[] { String.valueOf ( id ) } , null , null , null ,
				null );
		if ( cursor != null )
			cursor.moveToFirst ( );

		FillUp contact = new FillUp ( cursor.getString ( 0 ),
	...
	public List<FillUp> getAllFillUps()
	{
		...
		if ( cursor.moveToFirst ( ) ) {
			do {
				FillUp fu = new FillUp ( cursor.getString ( 0 ),
						cursor.getDouble ( 1 ), cursor.getDouble ( 2 ),
						cursor.getDouble ( 3 ), cursor.getDouble ( 4 ),
						cursor.getDouble ( 5 ), cursor.getString ( 6 ),
						cursor.getString ( 7 ) );
	...
	public int updateFillUp(FillUp fillUp)
	{
		...
		return db.update ( TABLE_FILL_UPS , values , KEY_CAR + " = ?" ,
				new String[] { String.valueOf ( fillUp.getCarName ( ) ) } );
	}

	public void deleteFillUp(FillUp fu, int row)
	{
		SQLiteDatabase db = this.getWritableDatabase ( );
		db.delete ( TABLE_FILL_UPS , KEY_DISTANCE + " = ?" ,
				new String[] { String.valueOf ( fu.distance ) } );
		db.close ( );
	}
